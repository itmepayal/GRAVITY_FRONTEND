...
import { X, Smile, Palette, Check, Lock, Globe, Plus } from "lucide-react";
import {
  type Workspace,
  WORKSPACE_COLORS,
  WORKSPACE_ICONS,
...
export const CreateWorkspaceModal = ({
  ...
  const [color, setColor] = useState("#6366F1"); // Mongoose schema default
  const [icon, setIcon] = useState("💼");
  const [isPrivate, setIsPrivate] = useState(false); // Mongoose schema default
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="border-b border-[#0F2D29]/8 bg-gradient-to-r from-[#8FE3C4]/15 via-transparent to-[#3FA9F5]/10 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="flex h-8 w-8 items-center justify-center rounded-lg text-[#8FA69E] transition hover:bg-[#0F2D29]/5 hover:text-[#0F2D29]"
            >
              <X size={16} />
            </button>
          </div>
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          <div>
            <label htmlFor="ws-name" className="mb-1.5 block text-[12px] font-semibold text-[#0F2D29]">
              Workspace Name *
            </label>
            <input
              id="ws-name"
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="ws-desc" className="text-[12px] font-semibold text-[#0F2D29]">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              {WORKSPACE_COLORS.map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() => setColor(c)}
                  className={`flex h-8 w-8 items-center justify-center rounded-xl transition-all ${color === c
          ...
          <div className="rounded-xl border border-[#0F2D29]/10 bg-[#0F2D29]/2 p-3.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-lg ${isPrivate
          ...
          <div className="flex justify-end gap-2.5 pt-3 border-t border-[#0F2D29]/8">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl px-4 py-2 text-[13px] font-medium text-[#5B6E68] transition hover:bg-[#0F2D29]/5"
            >
              Cancel
            </button>
            <button
